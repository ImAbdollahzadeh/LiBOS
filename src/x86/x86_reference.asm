...
section .text
    ...
    p_mode16: use16
        ...
        and  eax, ~0x01                        ; mask off PE bit to turn off protected mode
    ...
    ib: db 0x00
        ...
        mov  eax, cr0                          ; get cr0 so we can modify it
        or   eax, 0x01                         ; set PE bit to turn on protected mode
        mov  cr0, eax                          ; set cr0 to result
...
global __LiBOS_BiosCall_pg, _int32_pg
...
section .text
    ...
    p_mode16: use16
        ...
        mov  ss, ax                            ; set ss to 16bit selector
        mov  eax, cr0                          ; get cr0 so we can modify it
        and  eax, ~0x80000001                  ; mask off PE bit to turn off protected mode & PG bit to turn off paging
        mov  cr0, eax                          ; set cr0 to result
        jmp  word 0x0000:REBASE(r_mode16)      ; finally set cs:ip to enter real-mode
    r_mode16: use16
        xor  ax, ax                            ; set ax to zero
    ...
    ib: db 0x00
        ...
        call resetpic                          ; restore the pic's to protected mode settings
        mov  eax, cr0                          ; get cr0 so we can modify it
        or   eax, 0x80000001                   ; set PE bit to turn on protected mode & PG bit to turn on paging
        mov  cr0, eax                          ; set cr0 to result
        jmp  dword CODE32:REBASE(p_mode32)     ; switch to 32bit selector (32bit protected mode)
    p_mode32: use32
        mov  ax, DATA32                        ; get our 32bit data selector
