...
void convert_xor_instruction(TRIPLE_PACKET* tp, unsigned int* PC)
{
	...
	if(tp->mod1 == 'o' && tp->mod2 == 'o' && tp->mod3 == 'I') // A.K.A: xor, reg, imm 
	{
		...
		for(j=0;j<sizeof_opcodes;j++)
		{
			if( _strcmp(tp->str1, opcodes[j].mnemonic) )
			{
				if( attribute == opcodes[j].attribute )
				{
					...
					// This is the only part which makes XOR different than SUB or ADD
					modrm |= (6 << 3);
	...
	else if(tp->mod1 == 'o' && tp->mod2 == 'o' && tp->mod3 == 'M') // A.K.A: xor, reg, mem
	{
		//printf("MEM\n");
		unsigned char prefix = 0;
		unsigned char opc	= 0;
...
EXIT_POSITION:
	...
	else if(tp->mod1 == 'o' && tp->mod2 == 'M' && tp->mod3 == 'o') // A.K.A: xor, mem, reg
	{
		//printf("MEM\n");
		unsigned char prefix = 0;
		unsigned char opc	= 0;
...
EXIT_POSITION2:
	...
	else if(tp->mod1 == 'o' && tp->mod2 == 'M' && tp->mod3 == 'I') // A.K.A: xor, mem, imm
	{
		//printf("MEM+IMM\n");
		unsigned char prefix = 0;
		unsigned char opc	= 0;
	...
	else if(tp->mod1 == 'o' && tp->mod2 == 'o' && tp->mod3 == 'o') // A.K.A: xor, reg, reg
	{
		unsigned char         prefix    = 0;
		unsigned char         opc       = 0;
		unsigned char         modrm     = 0;
