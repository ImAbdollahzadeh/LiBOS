...
void convert_cmp_instruction(TRIPLE_PACKET* tp, unsigned int* PC)
{
	...
	if(tp->mod1 == 'o' && tp->mod2 == 'o' && tp->mod3 == 'I') // A.K.A: cmp, reg, imm 
	{
		...
		for(j=0;j<sizeof_opcodes;j++)
		{
			if( _strcmp(tp->str1, opcodes[j].mnemonic) )
			{
				if( attribute == opcodes[j].attribute )
				{
					...
					// This is the only part which makes CMP different than SUB or ADD
					modrm |= (7 << 3);
	...
	else if(tp->mod1 == 'o' && tp->mod2 == 'o' && tp->mod3 == 'M') // A.K.A: cmp, reg, mem
	{
		//printf("MEM\n");
		unsigned char prefix = 0;
		unsigned char opc	= 0;
...
EXIT_POSITION:
	...
	else if(tp->mod1 == 'o' && tp->mod2 == 'M' && tp->mod3 == 'o') // A.K.A: cmp, mem, reg
	{
		//printf("MEM\n");
		unsigned char prefix = 0;
		unsigned char opc	= 0;
...
EXIT_POSITION2:
	...
	else if(tp->mod1 == 'o' && tp->mod2 == 'M' && tp->mod3 == 'I') // A.K.A: cmp, mem, imm
	{
		//printf("MEM+IMM\n");
		unsigned char prefix = 0;
		unsigned char opc	= 0;
	...
	else if(tp->mod1 == 'o' && tp->mod2 == 'o' && tp->mod3 == 'o') // A.K.A: cmp, reg, reg
	{
		unsigned char         prefix    = 0;
		unsigned char         opc       = 0;
		unsigned char         modrm     = 0;
